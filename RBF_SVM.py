...
# Evaluate the model
cm = confusion_matrix(y_test, rbf_preds)
print("Optimized SVM with RBF Kernel Confusion Matrix:")
sns.heatmap(cm, annot=True, fmt='d', cmap='Blues', xticklabels=['Negative', 'Positive'], yticklabels=['Negative', 'Positive'])
plt.xlabel('Predicted')
plt.ylabel('Actual')
plt.title('Optimized SVM with RBF Kernel Confusion Matrix')
plt.show()

print("Optimized SVM with RBF Kernel Classification Report:")
print(classification_report(y_test, rbf_preds))

accuracy = accuracy_score(y_test, rbf_preds)
print(f"Optimized SVM with RBF Kernel Accuracy: {accuracy:.4f}")
